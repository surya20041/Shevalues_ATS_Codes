        ...
        try:
            with open(csv_file, newline='', encoding='utf-8') as f:
                ...
                for row in reader:
                    ...
                    if not (fname or mname or lname or email):
                        print(f"⚠️ Skipped row with all empty key fields in {csv_file}")
                        skipped_rows += 1
                        continue

                    # Create unique key
                    key = (fname, mname, lname, email)
...
print(f"⚠️ Skipped rows (all key fields empty): {skipped_rows}")